...
export default observer(function Page() {
  ...
  const [batchPostStatus, setbatchPostStatus] = useState("UnPosted");
  const [tableData, setTableData] = useState([]);
  const [batchMasterId, setBatchMasterId] = useState(0);

  const [batchMasterTable, setBatchMasterTable] = useState([]);
  ...
  const modalPopHandler = (batchMasterId: number) => {
    setIsModalOpen(true);
  ...
  const columns: ColumnsType<DataType> = [
    ...
    },
    {
      ...
      responsive: ["sm"],
      width: 110,
      dataIndex: "batchSpreadSheetMasterId",
      key: "batchSpreadSheetMasterId",
      sorter: true,
      render: (text, record: DataType) => (
        <div className="flex flex-col capitalize">
          <p
            className="text-[12px] cursor-pointer text-blue-700"
            onClick={() => modalPopHandler(record.batchSpreadSheetMasterId)}
          >
            {text}
          </p>
        </div>
    ...
    {
      title: <p className="text-center">InputedBy</p>,
      responsive: ["sm"],
      width: 130,
      dataIndex: "inputBy",
      key: "inputBy",
      render: (text) => (
        <p className="capitalize text-center">{text.toLowerCase()}</p>
      ),
    },
  ...
  const columnsTable: ColumnsType<DataType> = [
    {
      ...
      render: (text: any, record: DataType, index: number) => (
        <div className="flex flex-col capitalize gap-2">
          <p className="text-[12px] cursor-pointer font-[600]" key={index}>
            {text}
          </p>
          <p key={index} className="text-[11px]">
            {" "}
            {record.account?.accountName || "no Account Name"}
          </p>
        </div>
      ),
    },
  ...
  const fetch = async () => {
    ...
    const fetchTableData = await depositStore.getAllBatchPostingList(
      batchPostStatus
    );
  ...
  const fetchBatchChildren = async () => {
    setIsloading(true);
    if (batchMasterId >= 1) {
      const batchFetchResponse = await depositStore.getSingleBatchPost(
        batchMasterId
      );
      if (batchFetchResponse.kind === "ok") {
        setIsloading(false);
        setBatchMasterTable(batchFetchResponse.data.result.transactions);
      } else {
        setBatchMasterTable([]);
        setIsloading(false);
      }
    ...
    <Spin tip="fetching...." spinning={isloading} className="text-[red] ">
      <main className=" min-h-screen flex flex-col gap-1 w-[95%]  mx-auto ">
        <div className="flex justify-between items-center mb-3">
          ...
          />
          <Link href="/user/accounting/journal-posting/batch-posting/New-Batch-Post">
            <button
              type="button"
