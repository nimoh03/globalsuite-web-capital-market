...
export default observer(function AccountingCustomerDepositDebitNote() {
  const { depositStore } = useStores();

  const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([]);
  const [isloading, setIsloading] = useState(true);
  const [isOpen, setIsOpen] = useState(false);
  const [customerTransferDataStatus, setcustomerTransferDataStatus] =
    useState("UnPosted");
  ...
  const navigateToDetailsPage = (transNo: String) => {
    router.push(`/user/accounting/customer-deposit/customer-opening-balance/${transNo}`);
  };
        ...
        <div className="flex justify-evenly items-center gap-2">
          <Tooltip placement="bottom" title="approve">
            <Image
              ...
              onClick={() => navigateToDetailsPage(record?.transNo)}
            />
          </Tooltip>
          ...
          <Tooltip placement="bottom" title="edit">
            <Image
              ...
              onClick={() => navigateToEditPage(record?.transNo)}
            />
          </Tooltip>
  ...
  const saveStatusHandler=(status:string)=>{
    depositStore.setOpeningBalStatus(status);
}
  const fetch = async () => {
    setIsloading(true);
    saveStatusHandler(customerTransferDataStatus)
    const fetchTableData = await depositStore.getAllCustomerOpeningBal(
  ...
  const batchApproveHandler = (arr: { transNo: string }[]) => {
    setIsModalOpen(false);
    setTotalTransactionRecord(arr.length);
    setIsloading(true);
    arr.map(async (value) => {
      const res = await depositStore.approveCustomerOpeningBal(value.transNo);
      if (res.kind === "ok") {
        setIsModalOpen(false);
        setIsloading(false);
    ...
    <Spin tip="fetching...." spinning={isloading} className="text-[red] ">
      ...
      <div className="flex justify-end items-center gap-2">
            ...
            <button
              ...
              onClick={() => setcustomerTransferDataStatus("Reversed")}
            >
            ...
            <button
              type="button"
              className=" inline-flex font-[700] capitalize text-[#194BFB]  bg-[#EDF2F7] border-0 py-2 px-4 focus:outline-none hover:bg-[#d7d8da] rounded text-[16px]"
              onClick={() => setcustomerTransferDataStatus("Posted")}
            >
          ...
          <div className=" rounded-lg shadow-xl w-full ">
            <p className="flex justify-start gap-2 items-center font-bold text-[16px] leading-10 px-2 w-full capitalize bg-[#194bfb] text-white rounded-t-lg">
              ...
              {customerTransferDataStatus === "Unposted"
                ? "Unapproved"
                : customerTransferDataStatus === "Posted"
                ? "Approved"
                : customerTransferDataStatus === "Reversed"
                ? "Reversed"
                : null}{" "}
              Customer Deposits
            </p>
