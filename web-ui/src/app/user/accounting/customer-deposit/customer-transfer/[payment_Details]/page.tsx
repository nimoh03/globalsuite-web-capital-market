...
export default observer(function Page({
  ...
  const navigateToNewDepositPage = () => {
    router.push("/user/accounting/customer-deposit/customer-opening-balance/new-customer-openingBal");
  };
  const navigateToEditPage = (transNo: String) => {
    router.push(`/user/accounting/customer-deposit/customer-opening-balance/${transNo}/Edit`);
  };
  const fetch = async () => {
    const fetchTransactionDetails = await depositStore.getCustomerTransferByCode(
      params.payment_Details.toString(),depositStore.customerTransferDataStatus || localStorage.getItem('customerTransferDataStatus')
    );
  ...
  const approveTransactionHandler = async (code: string) => {
    ...
    setIsLoading(true);
    const res = await depositStore.approveCustomerOpeningBal(code);
    if (res.kind === "ok") {
      setIsLoading(false);
      depositStore.setOpeningBalStatus('Posted');
  ...
  return (
    <div className={`${manRope.variable} w-[90%] mx-auto  flex flex-col gap-5`}>
          ...
          <div className="flex gap-12 justify-between  items-center  py-5">
            ...
            <div className="text-center font-[500]   capitalize text-[15px] leading-tight">
              ...
              <p className="text-[#718096] font-[500] text-[18px]">
                deposited by :{" "}
                {TransactionDetails?.rCustomer &&
                  TransactionDetails.rCustomer?.fullName}
              </p>
              <p className="text-[#718096]">
                to : {" "}
                {TransactionDetails?.tCustomer &&
                  TransactionDetails?.tCustomer?.fullName}
              </p>
            </div>
            <span></span>
          </div>
            ...
            <div className="p-8 flex flex-col gap-1 shadow-lg">
              <div className="border-2 border-[#D8E3F8] grid grid-cols-2">
                <div className="grid grid-cols-2 justify-center items-center">
                  ...
                  <p className="p-2 font-[700] text-[16px] leading-[24px] text-[#2A313C] bg-[#D8E3F8]">
                    { TransactionDetails?.transNo || "null"}
                  </p>
                </div>
                <div className="grid grid-cols-2 justify-center items-center">
                  <p className="p-2 flex justify-start items-center gap-2 font-[500] text-[16px] leading-[24px] text-[#5D6A83]">
              ...
              <div className="border-2 border-[#D8E3F8] grid grid-cols-2">
                <div className="grid grid-cols-2 justify-center items-center">
                  ...
                  <p className="p-2 font-[700] text-[16px] leading-[24px] text-[#2A313C] bg-[#D8E3F8]">
                    {TransactionDetails?.effDate.slice(0, 10) || "null"}
                  </p>
                </div>
                <div className="grid grid-cols-2 justify-center items-center">
                  <p className="p-2 flex justify-start items-center gap-2 font-[500] text-[16px] leading-[24px] text-[#5D6A83]">
              ...
              <div className="border-2 border-[#D8E3F8] grid grid-cols-2">
                ...
                <div className="grid grid-cols-2 justify-center items-center">
                  ...
                  <p className="p-2 font-[700] text-[16px] leading-[24px] text-[#2A313C] bg-[#D8E3F8]">
                    {TransactionDetails?.description || "Empty"}
                  </p>
                </div>
              </div>
              ...
              <div className="flex justify-between items-center mt-5  ">
                <p className="text-[#128A3E] font-[700] text-[16px] leading-[20px] ">{`Email: ${
                  TransactionDetails?.rCustomer?.email
                } | Phone: ${
                  TransactionDetails.rCustomer?.phone ??
                  TransactionDetails.rCustomer?.mobPhone
                ...
                <div className="flex justify-between items-center gap-2">
                  ...
                  {TransactionDetails?.status === "UnPosted" ? (
                    <button
                      ...
                      className=" inline-flex font-[700] capitalize text-[#194BFB]  bg-[#EDF2F7] border-0 py-2 px-4 focus:outline-none hover:bg-[#d7d8da] rounded text-[16px]"
                      onClick={() =>
                        navigateToEditPage(TransactionDetails?.transNo)
                      }
                    >
                      Edit Deposit
                    </button>
      ...
      <Modal
        ...
        onOk={() => approveTransactionHandler(TransactionDetails.transNo)}
        ...
        footer={[
          <div key={2} className="  flex justify-center items-center">
            <Button
              ...
              onClick={() =>
                approveTransactionHandler(TransactionDetails.transNo)
              }
            >
              Approve
            </Button>
      ...
      <Modal
        ...
        okText="save"
        cancelText="Make Changes"
        onOk={() => reverseTransactionHandler(TransactionDetails.transNo)}
        ...
        footer={[
          <div key={7} className="  flex justify-center items-center">
            <Button
              ...
              className=" text-[16px] text-center w-[30%]  font-[500]"
              onClick={() =>
                reverseTransactionHandler(TransactionDetails.transNo)
              }
            >
              Reversed
            </Button>
        ...
        footer={[
          ...
          >
            <p className="text-[20px] font-bold leading-[26px] text-center capitalize">
              Transaction with ID : {TransactionDetails?.transNo || 'null'} Sucessfully
            </p>
          </div>,
        ]}
      >
