...
export default observer(function AccountingCustomerDepositDebitNote() {
  ...
  const navigateToDetailsPage = (transNo: String) => {
    router.push(`/user/accounting/customer-deposit/customer-transfer/${transNo}`);
  };
  const navigateToNewDepositPage = () => {
    router.push(`/user/accounting/customer-deposit/customer-transfer/new-customer-transfer`);
  };
  interface DataType {
    key: React.Key;
    TxnID: string;
  ...
  const saveStatusHandler=(status:string)=>{
    depositStore.setCustomerTransfer(status);
}
  const fetch = async () => {
    setIsloading(true);
    saveStatusHandler(customerTransferDataStatus)
    const fetchTableData = await depositStore.getAllCustomerTransfer(
  ...
  const batchApproveHandler = (arr: { transNo: string }[]) => {
    ...
    setIsloading(true);
    arr.map(async (value) => {
      const res = await depositStore.approveCustomerTransfer(value.transNo);
      if (res.kind === "ok") {
        setIsModalOpen(false);
        setIsloading(false);
...
<button
              type="button"
              className=" inline-flex font-[700] capitalize text-[#194BFB]  bg-[#EDF2F7] border-0 py-2 px-2 focus:outline-none hover:bg-[#d7d8da] rounded text-[16px]"
              onClick={() => setcustomerTransferDataStatus("UnPosted")}
            >
