...
export default observer(function AccountingCustomerDepositCreditNote() {
  ...
  const [debitNoteStatus, setdebitNoteStatus] = useState("UnPosted");
  ...
  const navigateToDetailsPage = (debitNo: String) => {
    router.push(`/user/accounting/customer-deposit/debit-note/${debitNo}`);
  };
  ...
  const columns: ColumnsType<DataType> = [
    ...
    },
    {
      ...
      dataIndex: "debitNo",
      key: "debitNo",
      sorter: (a: any, b: any) => a.debitNo.length - b.debitNo.length,
      render: (text) => (
        ...
        <div className="flex justify-evenly items-center gap-2">
          <Tooltip placement="bottom" title="approve">
            <Image
              ...
              onClick={() => navigateToDetailsPage(record.debitNo)}
            />
          </Tooltip>
          ...
          <Tooltip placement="bottom" title="edit">
            <Image
              ...
              onClick={() => navigateToEditDeposit(record.debitNo)}
            />
          </Tooltip>
  ...
  const saveStatusHandler=(status:string)=>{
    depositStore.setDebitNoteStatus(status);
}
  const fetch = async () => {
    setIsloading(true);
    saveStatusHandler(debitNoteStatus)
    const fetchTableData = await depositStore.getAllDebitNote(debitNoteStatus);
  ...
  const batchApproveHandler = (arr: { debitNo: string }[]) => {
    setIsModalOpen(false);
    setTotalTransactionRecord(arr.length);
    setIsloading(true);
    arr.map(async (value) => {
      const res = await depositStore.approveDebitNoteTransaction(value.debitNo);
      if (res.kind === "ok") {
        setIsModalOpen(false);
        setIsloading(false);
      ...
      <main className=" min-h-screen flex flex-col gap-1 w-[95%]  mx-auto ">
        ...
        <div className="flex justify-end items-center gap-2">
            ...
            <button
              ...
              onClick={() => setdebitNoteStatus("Reversed")}
            >
            ...
            <button
              type="button"
              className=" inline-flex font-[700] capitalize text-[#194BFB]  bg-[#EDF2F7] border-0 py-2 px-4 focus:outline-none hover:bg-[#d7d8da] rounded text-[16px]"
              onClick={() => setdebitNoteStatus("Posted")}
            >
              view Approved deposit
            </button>
            <Link href="/user/accounting/customer-deposit/debit-note/Create_New_Debit_Note">
            <button
              type="button"
        ...
        <div className=" rounded-lg shadow-xl w-full ">
            <p className="flex justify-start gap-2 items-center font-bold text-[16px] leading-10 px-2 w-full capitalize bg-[#194bfb] text-white rounded-t-lg">
              ...
              {debitNoteStatus === "Unposted"
                ? "Unapproved"
                : debitNoteStatus === "Posted"
                ? "Approved"
                : debitNoteStatus === "Reversed"
                ? "Reversed"
                : null}{" "}
              Customer Deposits
            </p>
