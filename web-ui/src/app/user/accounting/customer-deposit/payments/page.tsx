...
export default observer(function AccountingCustomerDeposit() {
  ...
  const navigateToDetailsPage = (code: String) => {
    router.push(`/user/accounting/customer-deposit/payments/${code}`);
  };

  const navigateToEditPage = (code: string) => {
    router.push(`/user/accounting/customer-deposit/payments/${code}/Edit`);
  };

  const addNewDeposit = () => {
    router.push(ROUTES.accountingCustomerDepositPaymentCreate().path);
  };
  const columns: ColumnsType<DataType> = [
    {
      title: (
        <p className="font-[800] text-[16px] leading-[24px] text-center">
          Date
        </p>
      ),
      responsive: ["sm"],
      ...
      key: "rnDate",
      render: (text: String) => (
        <p className="font-[400] text-[14px]">{text.slice(0, 10)}</p>
      ),
    },
    {
      title: (
      ...
      sorter: true,
      render: (text) => (
        <p className="text-[14px] font-[600] leading-[18.2px]">
          {text.fullName.replace(".", "")} - {text.custAID}
        </p>
      ),
    },
    {
      ...
      render: (text) => (
        <p className="capitalize font-[600] text-[14px]">
          {" "}
          {text.toLocaleString()}
        </p>
      ),
    ...
    {
      title: (
        <p className="font-[800] text-[16px] leading-[24px] text-center uppercase">
          Txn ID
        </p>
      ),
      responsive: ["sm"],
      ...
      dataIndex: "paymentNo",
      key: "paymentNo",
      sorter: (a: any, b: any) => a.paymentNo.length - b.paymentNo.length,
      render: (text) => <p className="capitalize text-[14px]">{text}</p>,
    },
        ...
        <div className="flex justify-evenly items-center gap-2">
          <Tooltip placement="bottom" title="approve">
            <Image
              ...
              onClick={() => navigateToDetailsPage(record.paymentNo)}
            />
          </Tooltip>
          ...
          <Tooltip placement="bottom" title="edit">
            <Image
              ...
              onClick={() => navigateToEditPage(record.paymentNo)}
            />
          </Tooltip>
  ...
  const fetch = async () => {
    ...
    const fetchTableData = await depositStore.getAllPayment(depositStatus);
  ...
  const batchApproveHandler = (arr: { paymentNo: string }[]) => {
    setIsModalOpen(false);
    setTotalTransactionRecord(arr.length);
    setIsloading(true);
    arr.map(async (value) => {
      const res = await depositStore.approvePaymentTransaction(value.paymentNo);
      if (res.kind === "ok") {
        setIsModalOpen(false);
        setIsloading(false);
  ...
  return (
    <>
      ...
      >
        <div className="w-[90%] mx-auto justify-center items-center text-center mt-8 gap-5">
