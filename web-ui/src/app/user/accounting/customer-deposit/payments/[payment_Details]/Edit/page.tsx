...
export default observer(function Page({
  ...
  const [allSubBank, setallSubBank]: any = useState([]);
  ...
  const fetch = async (
    ...
    if (
      ...
      form.setFieldsValue({
        acctMasBank: fetchTransactionDetails.data.result?.accoutMasBank?.productCode,
        branch: fetchTransactionDetails.data.result?.branchDetail?.transNo,
        instrumentType:fetchTransactionDetails.data.result?.instrumentType,
        code :fetchTransactionDetails.data.result?.paymentNo,
        ...
        transdesc :fetchTransactionDetails.data.result?.transDesc,
        doNotChargeBankStampDuty: fetchTransactionDetails.data.result?.doNotChargeBankStampDuty,
...
return ({
  ...
  const handleSubmit = async () => {
    ...
    if (await form.validateFields({ validateOnly: true })) {
      setIsModalOpen(false);
      setIsLoading(true);
      const response = await depositStore.updatePaymentTransaction({
        ...values,
        ...
        // payDesc: "",
        custNo: CustomerNo,
        doNotChargeBankStampDuty: false
      });
      if (response.kind === "ok") {
          ...
          footer={[
            ...
            >
              ...
              <div className="flex flex-col justify-start items-start">
                ...
                <p className="text-sm font-[400]">
                  Transaction No : {depositSucessResponse.paymentNo}
                </p>
              </div>
            </div>,
          ]}
