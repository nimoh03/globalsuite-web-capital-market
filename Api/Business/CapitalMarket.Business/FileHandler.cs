    ...
    public class FileHandler
    {
        ...
        public bool ReadTextFile(string filePath,out List<string> oCustMissings,out List<string> oStockMissings,out List<string> oBrokerMissings)
        {
            ...
            try
            {
                ...
                IEnumerable<string> oProductAccts = oProductAcct.GetAllByProduct(oStkParam.Product,"ALL").Tables[0].AsEnumerable().Select(dataRow => dataRow.Field<string>("CsCs Account") != null ? dataRow.Field<string>("CsCs Account").Trim() : "");
                ...
                using (StreamReader sr = new StreamReader(filePath))
                {
                    ...
                    while ((strline = sr.ReadLine()) != null)
                    {
                        if (strline != "")
                        {
                            ...
                            if (details[0] == null || details[0].Trim(removedata) == "Board" || details[0].Trim(removedata) == "BOARD")
                            { }
                            else
                            {
