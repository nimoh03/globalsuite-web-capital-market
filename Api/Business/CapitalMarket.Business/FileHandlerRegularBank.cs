...
{
    public class FileHandlerRegularBank
    {
        #region Declartion
        ...
        public bool ReadTextFile(string filePath,out List<string> oCustMissings,out List<string> oStockMissings,out List<string> oBrokerMissings)
        {
            ...
            try
            {
                ...
                {
                    oBargainTrans.Bslip = oTradeFileRow.TicketNumber;
                    oBargainTrans.iDate = oTradeFileRow.TransactionDate;
                    oBargainTrans.StockCode = oTradeFileRow.SecurityName;
                    oBargainTrans.Units = oTradeFileRow.Quantity;
                    oBargainTrans.UnitPrice = oTradeFileRow.Price;
                    oBargainTrans.Consideration = oTradeFileRow.Consideration;
                    ...
                    if (oTradeFileRow.BuySell == "1")
                    {
                        ...
                        oBargainTrans.SoldBy = oTradeFileRow.CounterParty;
                    }
                    else if (oTradeFileRow.BuySell == "2")
                    {
                        oBargainTrans.SoldBy = oCompany.MemberCode;
                        oBargainTrans.Buy_Sold_Ind = "S";
                        oBargainTrans.BoughtBy = oTradeFileRow.CounterParty;
                    }
                    if (oBargainTrans.BoughtBy.Trim() == oBargainTrans.SoldBy.Trim())
                    {
                        oBargainTrans.CrossD = "Y";
                    }
                    else
                    {
                        oBargainTrans.CrossD = "N";
                    }
                    oBargainTrans.AcctNo = oTradeFileRow.CscsAccount;
        ...
        #region Reading Text File Customer Bank Margin
        public bool ReadTextFileCustomerBankMargin(string filePath, out List<string> oCustMissings, out List<string> oStockMissings, out List<string> oBrokerMissings)
        {
            bool blnStatus = false;
            oCustMissings = null;
            oStockMissings = null;
                        ...
                        if (strline != "")
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    oTradeFileSecond.Quantity = int.Parse(details[11].Trim(removedata));
                                    oTradeFileSecond.Price = Decimal.Parse(details[10].Trim(removedata));
                                    oTradeFileSecond.Consideration = long.Parse(details[11].Trim(removedata)) * Decimal.Parse(details[10].Trim(removedata));
