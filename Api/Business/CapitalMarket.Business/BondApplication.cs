...
namespace CapitalMarket.Business
{
    public class BondApplication
    {
        ...
        private long lngTransNo, lngTransNoRev,lngBondOffer,lngUnits;
        private decimal decAmount;
        private bool blnResident, blnResident2, blnPosted, blnReversed;
        private string strCustomerId, strBondAccount, strApplicantType;
        ...
        public long BondOffer
        {
            set { lngBondOffer = value; }
            get { return lngBondOffer; }
        }
        ...
        public DataGeneral.SaveStatus Save()
        {
            ...
            using (SqlConnection connection = db.CreateConnection() as SqlConnection)
            {
                ...
                try
                {
                    ...
                    {
                        dbCommand = db.GetStoredProcCommand("BondApplicationAddNew") as SqlCommand;
                    }
                    else if (strSaveType.Trim() == "EDIT")
                    {
                        dbCommand = db.GetStoredProcCommand("BondApplicationEdit") as SqlCommand;
                    }
                    ...
                    db.AddInParameter(dbCommand, "BondOffer", SqlDbType.BigInt, lngBondOffer);
                    db.AddInParameter(dbCommand, "Units", SqlDbType.BigInt, lngUnits);
        ...
        public bool ChkTransNoExist(DataGeneral.PostStatus ePostStatus)
        {
            ...
            if (strSaveType == "EDIT")
            {
                ...
                {
                    oCommand = db.GetStoredProcCommand("BondApplicationChkTransNoExistUnPosted") as SqlCommand;
                }
        ...
        #region Get Bond Application
        public bool GetBondApplication(DataGeneral.PostStatus ePostStatus)
        {
            ...
            if (ePostStatus == DataGeneral.PostStatus.UnPosted)
            {
                dbCommand = db.GetStoredProcCommand("BondApplicationSelectUnPosted") as SqlCommand;
            }
            else if (ePostStatus == DataGeneral.PostStatus.Posted)
            {
                dbCommand = db.GetStoredProcCommand("BondApplicationSelectPosted") as SqlCommand;
            }
            else if (ePostStatus == DataGeneral.PostStatus.Reversed)
            {
                dbCommand = db.GetStoredProcCommand("BondApplicationSelectReversed") as SqlCommand;
            }
            db.AddInParameter(dbCommand, "TransNo", SqlDbType.BigInt, lngTransNo);
            ...
            if (thisRow.Length == 1)
            {
                ...
                lngBondOffer = long.Parse(thisRow[0]["BondOffer"].ToString());
                lngUnits = long.Parse(thisRow[0]["Units"] != null  && thisRow[0]["Units"].ToString().Trim() != "" ? thisRow[0]["Units"].ToString() : "0");
        ...
        public DataSet GetAll(DataGeneral.PostStatus TransStatus)
        {
            ...
            {
                dbCommand = db.GetStoredProcCommand("BondApplicationSelectAllUnPosted") as SqlCommand;
            }
            else if (TransStatus == DataGeneral.PostStatus.Posted)
            {
                dbCommand = db.GetStoredProcCommand("BondApplicationSelectAllPosted") as SqlCommand;
            }
            else if (TransStatus == DataGeneral.PostStatus.Reversed)
            {
                dbCommand = db.GetStoredProcCommand("BondApplicationSelectAllReversed") as SqlCommand;
            }
            else if (TransStatus == DataGeneral.PostStatus.PostedAsc)
            {
                dbCommand = db.GetStoredProcCommand("BondApplicationSelectAllPostedAsc") as SqlCommand;
            }
            else if (TransStatus == DataGeneral.PostStatus.UnPostedAsc)
            {
                dbCommand = db.GetStoredProcCommand("BondApplicationSelectAllUnPostedAsc") as SqlCommand;
            }
            DataSet oDS = db.ExecuteDataSet(dbCommand);
            return oDS;
        }
        ...
        public bool Delete()
        {
            bool blnStatus = false;
            DatabaseProviderFactory factory = new DatabaseProviderFactory(); SqlDatabase db = factory.Create("GlobalSuitedb") as SqlDatabase;
            SqlCommand oCommand = db.GetStoredProcCommand("BondApplicationDelete") as SqlCommand;
            db.AddInParameter(oCommand, "TransNo", SqlDbType.BigInt, lngTransNo);
        ...
        #region Get Total Number Of Returns For A Particular Bond Offer Real
        public decimal GetTotalBondOfferApp()
        {
            DatabaseProviderFactory factory = new DatabaseProviderFactory(); SqlDatabase db = factory.Create("GlobalSuitedb") as SqlDatabase;
            SqlCommand dbCommand = db.GetStoredProcCommand("BondApplicationSelectByBondOfferCodeReturnSumAmount") as SqlCommand;
            db.AddInParameter(dbCommand, "BondOffer", SqlDbType.BigInt,lngBondOffer);
        ...
        {
            DatabaseProviderFactory factory = new DatabaseProviderFactory(); SqlDatabase db = factory.Create("GlobalSuitedb") as SqlDatabase;
            SqlCommand oCommand = db.GetStoredProcCommand("BondApplicationDeleteReversal") as SqlCommand;
